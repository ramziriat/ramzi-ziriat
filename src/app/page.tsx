...
/* ============================================================
   ...
   ============================================================ */
const VORONOI_EVERY = 4;
const REGION_COUNT  = 5;

const REGION_DEFS = [
  { name: "Astrophysics", hue: 200, labels: ["Pulsars", "Neutron Stars", "Observation", "Spectroscopy", "Astrophotography"] },
  { name: "Cosmology",    hue: 260, labels: ["Dark Matter", "CMB", "Inflation", "Large-Scale Structure", "Redshift"] },
  { name: "Aerospace",    hue: 170, labels: ["Propulsion", "Orbital Mechanics", "Re-entry", "Avionics", "Structures"] },
  { name: "Philosophy",   hue: 300, labels: ["Epistemology", "Science Ethics", "Consciousness", "Logic", "Metaphysics"] },
  { name: "Aviation",     hue: 210, labels: ["PPL", "Navigation", "Meteorology", "Flight Dynamics", "Airspace"] },
];

const INACTIVE_PER_REGION = 14;

// Mouse-distance thresholds for node sizing
const D_ON_NODE  = 22;   // mouse directly on node → very large
const D_NEAR     = 70;   // close → 2× size
const D_MEDIUM   = 140;  // medium → 1.35× size
const ATTRACT_R  = 280;  // beyond this mouse has no pull
...
function ellipseLineT(
  mx: number, my: number, dx: number, dy: number,
  cx: number, cy: number, rx: number, ry: number
): [number, number] | null {
  const ax = (mx-cx)/rx, ay = (my-cy)/ry;
  const bx = dx/rx, by = dy/ry;
  const a = bx*bx+by*by, b = 2*(ax*bx+ay*by), c = ax*ax+ay*ay-1;
...
function NeuralNetwork() {
  ...
  useEffect(() => {
    ...
    /* ---- Mouse: attach to section so overlay elements don't block it ---- */
    ...
    REGION_DEFS.forEach((rDef, rid) => {
      ...
      rDef.labels.forEach((label, li) => {
        ...
        nodes.push({
          ...
          orbitAmp:  (8 + Math.random() * 14) * SCALE,
          pulse: 0, activity: 0,
        });
      });
    ...
    const drawRegionBoundaries = () => {
      ...
      for (let i = 0; i < REGION_COUNT; i++) {
        const a = regions[i], b = regions[(i+1) % REGION_COUNT];
    ...
    /* ---- Pulse propagation ----
       When the mouse hovers a node, we fire waves to the nearest
       3-4 active neighbours with decaying intensity. Each wave
       hop then propagates one more level at further reduced strength.
       This gives a ripple of size pulses spreading outward.
    ---- */
    interface Pulse { node: any; strength: number; delay: number; depth: number; from: any }
    let pulses: Pulse[] = [];
    let hoveredNodeId = -1;

    const triggerPulse = (origin: any) => {
      // Find nearest 4 active neighbours by distance
      const activeOthers = nodes.filter(n => n.id !== origin.id && n.active);
      activeOthers.sort((a: any, b: any) => {
        const da = (a.x-origin.x)**2 + (a.y-origin.y)**2;
        const db = (b.x-origin.x)**2 + (b.y-origin.y)**2;
        return da - db;
      });
      const neighbours = activeOthers.slice(0, 4);
      neighbours.forEach((nb: any, i: number) => {
        const dist = Math.sqrt((nb.x-origin.x)**2 + (nb.y-origin.y)**2);
        pulses.push({
          node: nb,
          strength: 0.9 - i * 0.12,
    ...
    const processPulses = () => {
      for (let i = pulses.length - 1; i >= 0; i--) {
        ...
        // Small outward push from source
        const dx = n.x - p.from.x, dy = n.y - p.from.y;
        const d  = Math.sqrt(dx*dx + dy*dy) || 1;
        n.vx += (dx/d) * p.strength * 0.5;
        n.vy += (dy/d) * p.strength * 0.5;
        pulses.splice(i, 1);
        // One more hop with heavy decay, max depth 2
        if (p.depth < 2 && p.strength > 0.25) {
          const others = nodes.filter((o: any) => o.id !== n.id && o.active);
          others.sort((a: any, b: any) =>
            (a.x-n.x)**2+(a.y-n.y)**2 - (b.x-n.x)**2-(b.y-n.y)**2
          );
          others.slice(0, 3).forEach((nb: any) => {
            const dist = Math.sqrt((nb.x-n.x)**2+(nb.y-n.y)**2);
            if (dist < 180) {
              pulses.push({
                node: nb,
                strength: p.strength * 0.45,
                delay: Math.max(3, Math.round(dist / 12)),
                depth: p.depth + 1,
                from: n,
              });
            }
    ...
    /* ---- Voronoi region containment ----
       Check the node against EVERY other region seed, not just angular
       neighbours. For each rival region, compute the Voronoi bisector
       (midpoint of own-seed↔rival-seed), project the node onto it, and
       nudge it back if it has crossed over. Because the region seed is
       the live average of all its member nodes, the boundary itself
       adapts — so the region expands/contracts to keep its nodes rather
       than teleporting them. Nodes from different regions that get too
       close are also pushed apart to prevent stacking.
    ---- */
    const REGION_MARGIN = 8;
    const enforceContainment = (n: any) => {
      const own = regions[n.regionId].seed;
      for (const other of regions) {
        if (other.id === n.regionId) continue;
        const ox = other.seed.x, oy = other.seed.y;
        const rx = own.x, ry2 = own.y;
        const dx = ox - rx, dy = oy - ry2;
        const dLen2 = dx*dx + dy*dy;
        if (dLen2 < 1) continue;
        const mx = (rx + ox) / 2, my = (ry2 + oy) / 2;
        ...
        if (proj > -marginTerm) {
          ...
          const factor = (excess / dLen2) * 0.55;
          n.x -= dx * factor;
          n.y -= dy * factor;
          n.vx *= 0.55; n.vy *= 0.55;
        }
    ...
    /* ---- Anti-stacking: push nodes apart if too close ---- */
    const MIN_NODE_DIST = 18;
    const avoidStacking = () => {
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          const d2 = dx*dx + dy*dy;
          if (d2 < MIN_NODE_DIST * MIN_NODE_DIST && d2 > 0.001) {
            const d = Math.sqrt(d2);
            const f = (MIN_NODE_DIST - d) / d * 0.5;
            const fx = dx * f, fy = dy * f;
            a.x -= fx; a.y -= fy;
            b.x += fx; b.y += fy;
            a.vx -= fx * 0.3; a.vy -= fy * 0.3;
            b.vx += fx * 0.3; b.vy += fy * 0.3;
          }
        }
      }
    };
    ...
    /* ---- Update home positions (called every frame after globalAngle increments) ---- */
    const updateHomes = () => {
      const c = center(), rx = eRX(), ry = eRY();
      nodes.forEach((n: any) => {
        const rotAngle  = n.regionAngle + globalAngle;
        const nodeAngle = n.spreadAngle + globalAngle;
        n.homeX = c.x + Math.cos(rotAngle) * rx * 0.45 + Math.cos(nodeAngle) * n.spreadR;
        n.homeY = c.y + Math.sin(rotAngle) * ry * 0.45 + Math.sin(nodeAngle) * n.spreadR;
      });
    };
    ...
    resize();
    updateHomes();
    nodes.forEach((n: any) => { n.x = n.homeX; n.y = n.homeY; });
    updateSeeds();
    window.addEventListener("resize", () => { resize(); updateHomes(); });
    ...
    const draw = () => {
      ...
      for (const n of nodes) {
        ...
        // Mouse attraction with quadratic falloff
      ...
      // Run stacking avoidance before containment so containment has final say
      avoidStacking();

      // Enforce every node stays inside its own Voronoi region
      for (const n of nodes) enforceContainment(n);

      /* ---- Hover detection: find nearest active node within D_ON_NODE ---- */
      let nearestId = -1, nearestD = Infinity;
      for (const n of nodes) {
        if (!n.active) continue;
        const dx = mouse.x - n.x, dy = mouse.y - n.y;
        const d  = Math.sqrt(dx*dx + dy*dy);
        if (d < D_ON_NODE && d < nearestD) { nearestD = d; nearestId = n.id; }
      }
      ...
      /* ---- Build & draw edges ---- */
      const activeEdges: [any, any, number][] = [];
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          const a = nodes[i], b = nodes[j];
      ...
      for (let i = signals.length - 1; i >= 0; i--) {
        ...
        const t2 = Math.max(0, s.t - 0.15);
        const x2 = s.a.x + (s.b.x-s.a.x) * t2;
        const y2 = s.a.y + (s.b.y-s.a.y) * t2;
      ...
      /* ---- Draw nodes with tiered size response to mouse ---- */
      for (const n of nodes) {
        const mdx  = mouse.x - n.x, mdy = mouse.y - n.y;
        const dist = Math.sqrt(mdx*mdx + mdy*mdy);

        const baseSize = n.active ? 5.5 : 2.5;

        // Tiered size multiplier based on mouse proximity
        let sizeMult = 1.0;
        if (dist < D_ON_NODE) {
          sizeMult = 3.5;                                                // directly on node
        } else if (dist < D_NEAR) {
          const t = (dist - D_ON_NODE) / (D_NEAR - D_ON_NODE);
          sizeMult = 3.5 - t * 1.5;                                     // 3.5 → 2.0
        } else if (dist < D_MEDIUM) {
          const t = (dist - D_NEAR) / (D_MEDIUM - D_NEAR);
          sizeMult = 2.0 - t * 0.65;                                    // 2.0 → 1.35
        }

        // Pulse adds extra pop on top
        const size = baseSize * sizeMult * (1 + n.pulse * 0.8);

        n.pulse    *= 0.90;
        n.activity *= 0.90;

        const hue     = REGION_DEFS[n.regionId].hue;
        const onNode  = dist < D_ON_NODE;
        const nearLbl = dist < D_MEDIUM;

        if (n.active) {
          ctx.save();
          ctx.shadowBlur  = 14 + n.activity * 14;
        ...
        // Label appears when mouse is at medium range or closer
        if (n.active && nearLbl) {
          ctx.save();
          ctx.fillStyle = onNode ? "white" : `hsla(${hue},60%,90%,0.85)`;
          ctx.font      = onNode ? "bold 13px system-ui" : "11px system-ui";
          ctx.fillText(n.label, n.x + size + 6, n.y + 4);
          ctx.restore();
        }
      }
    };
  ...
  return (
    <section className="neuralSection" ref={sectionRef}>
...
function TimelinePage() {
  ...
  const getPage = (i: number) => entryPage[i] ?? 0;
    ...
    <section className="timelineSection">
      ...
      <div className="timelineTrack">
        ...
        {missions.map((m, i) => {
          ...
          const pg   = getPage(i);
          const entry = m.entries[pg] ?? m.entries[0];
          ...
          return (
            <div
              ...
              className={`tlNode ${entry.status.toLowerCase()} ${isHovered ? "tlHovered" : ""}`}
              style={{
                left: `${xPct}%`,
                top:  `${yPct}%`,
                ["--hue" as any]: m.hue,
                /* Raise z-index on hover so the card sits above everything */
            ...
            >
              ...
              {m.entries.some(e => e.status === "ACTIVE") && (
                <div className="tlPulseRing" />
              )}
              ...
              {/* Hover card — rendered in DOM only when hovered */}
              {isHovered && (
                <div
                  className={`tlCard ${cardAbove ? "above" : "below"}`}
                  /* Stop mouse-leave from firing when cursor moves into the card */
                  ...
                  {m.entries.length > 1 && (
                    <div className="tlCardPagination">
                      {m.entries.map((_, p) => (
                        <button
                          key={p}
                          className={`tlPageDot ${p === pg ? "active" : ""}`}
                          onMouseEnter={e => {
                            e.stopPropagation();
                            setEntryPage(prev => ({ ...prev, [i]: p }));
                          }}
                          onClick={e => {
                            e.stopPropagation();
                            setEntryPage(prev => ({ ...prev, [i]: p }));
  ...
  return (
    <main>
      ...
      <div className="viewport" style={{ transform: `translateY(-${page * 100}vh)` }}>
        ...
        {/* PAGE 1 — Neural network (section + canvas now live inside NeuralNetwork) */}
        <NeuralNetwork />
