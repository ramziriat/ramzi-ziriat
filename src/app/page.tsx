...
function NeuralNetwork() {
  ...
  useEffect(() => {
    ...
    /* --- Build regions & assign nodes randomly --- */
    ...
    let grid = new Uint8Array(
      Math.ceil(canvas.width / GRID_RES) * Math.ceil(canvas.height / GRID_RES)
    );

    const updateGrid = () => {
      const W = Math.ceil(canvas.width / GRID_RES);
      const H = Math.ceil(canvas.height / GRID_RES);
      if (grid.length !== W * H) grid = new Uint8Array(W * H);

      // Update seeds = centroid of member nodes
      ...
      for (let gy = 0; gy < H; gy++) {
        for (let gx = 0; gx < W; gx++) {
          ...
          for (const r of regions) {
            const dx = px - r.seed.x;
            const dy = py - r.seed.y;
    ...
    const drawVoronoiFills = () => {
      ...
      for (let py = 0; py < canvas.height; py++) {
        for (let px = 0; px < canvas.width; px++) {
          const gx = Math.min(Math.floor(px / GRID_RES), W - 1);
          const gy = Math.min(Math.floor(py / GRID_RES), H - 1);
          const rid = grid[gy * W + gx];
          const hue = regions[rid].hue;
          const [r, g, b] = hslToRgb(hue / 360, 0.45, 0.12);
          const idx = (py * canvas.width + px) * 4;
          d[idx] = r; d[idx + 1] = g; d[idx + 2] = b; d[idx + 3] = 28;
        }
      }
      ctx.putImageData(imageData, 0, 0);
    };

    const drawVoronoiBorders = () => {
      const W = Math.ceil(canvas.width / GRID_RES);
      const H = Math.ceil(canvas.height / GRID_RES);
      ctx.lineWidth = 1.5;
      ctx.setLineDash([4, 6]);
      for (let gy = 0; gy < H - 1; gy++) {
        for (let gx = 0; gx < W - 1; gx++) {
          ...
          if (c !== cr) {
            const avg = (regions[c].hue + regions[cr].hue) / 2;
            ctx.strokeStyle = `hsla(${avg},70%,75%,0.35)`;
            ctx.beginPath();
          ...
          if (c !== cb) {
            const avg = (regions[c].hue + regions[cb].hue) / 2;
            ctx.strokeStyle = `hsla(${avg},70%,75%,0.35)`;
            ctx.beginPath();
    ...
    const draw = () => {
      ...
      if (frame % VORONOI_INTERVAL === 0) updateGrid();
      drawVoronoiFills();
      drawVoronoiBorders();
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (d < 190) {
            ...
            const hue = regions[a.regionId].hue;
            ctx.strokeStyle = sameRegion
              ? `hsla(${hue},70%,70%,0.18)`
              : "rgba(76,201,240,0.07)";
            ctx.lineWidth = sameRegion ? 1 : 0.5;
            ctx.beginPath();
            ctx.moveTo(a.x, a.y);
            ctx.lineTo(b.x, b.y);
            ctx.stroke();
      ...
      for (const n of nodes) {
        ...
        const hue = regions[n.regionId].hue;
        ctx.beginPath();
        ctx.fillStyle = n.active
          ? `hsla(${hue},80%,72%,${0.85 + n.activity * 0.15})`
          : `hsla(${hue},40%,75%,0.45)`;
        ctx.arc(n.x, n.y, size, 0, Math.PI * 2);
        ctx.fill();

        if (n.active && near) {
          ctx.fillStyle = isHover ? "white" : "rgba(255,255,255,0.75)";
          ctx.font = isHover ? "14px system-ui" : "11px system-ui";
